...
HOST = "http://kqthpt.tayninh.edu.vn"
TMP_DIR = Path("./tmp/")
...
def ocr_captcha(ocr: PaddleOCR, session: Session) -> str:
    ...
    captcha_rsp = session.get(
        f"{HOST}/TraCuu/GetCaptcha?time={current_timestamp}&choose=1"
    )
    captcha_img_path = TMP_DIR.joinpath(f"{current_timestamp}.png")
...
def tra_cuu(session: Session, sbd: int, captcha_text: str):
    # Tây Ninh region is 46
    data = {"SOBAODANH": "46" + str(sbd).zfill(6), "ConfirmCode": captcha_text}
    tra_cuu_rsp = session.post(f"{HOST}/TraCuu/TraCuu", data=data)
...
def run(ocr: PaddleOCR, session: Session):
    student_id = 1
    file = DATA_DIR.joinpath("latest.csv")
    if file.exists():
    ...
    while True:
        ...
        if (
            tra_cuu_rsp is None
            or tra_cuu_rsp.get("Message") == "Không tìm thấy thí sinh"
        ):
            student_id += 1
            if student_id < 2000:
                ...
                student_attempt += 1
                continue
        if tra_cuu_rsp.get("Message") == "Mã xác nhận không hợp lệ":
        ...
        all_score_str = tra_cuu_rsp.get("DIEM_THI")
